...
void handle_connection(simple_connection *conn) {
    char buffer[1024];
    int ret;
    ...
    ret = recv(conn->fd, buffer, sizeof(buffer) - 1, MSG_DONTWAIT);
    if (ret > 0) {
        buffer[ret] = '\0';
...
int main(int argc, char **args)
{
    ...
    struct telnet_info info;
    int line_count = 0;
    simple_connection connections[50]; // Max 50 concurrent connections
    int conn_index = 0;
    ...
    while(TRUE)
    {
        ...
        else
        {
            ...
            if (slot == -1) {
                printf("[DEBUG] No available connection slots\n");
                continue;
            }
            ...
            if (fd != -1) {
                ...
                connections[slot].info = info;
                connections[slot].start_time = time(NULL);
                connections[slot].connected = 1;
                ...
                handle_connection(&connections[slot]);
            }
