...
void handle_connection(int fd, struct telnet_info *info) {
    ...
    if (ret <= 0) {
        printf("[DEBUG] No data received or timeout\n");
        close(fd);
    ...
    if (ret > 0) {
        ...
        if (strstr(buffer, "login") || strstr(buffer, "Login") || strstr(buffer, "ogin")) {
            ...
            if (ret > 0) {
                ...
                if (ret > 0) {
                    ...
                    if (strstr(buffer, "password") || strstr(buffer, "Password") || strstr(buffer, "assword")) {
