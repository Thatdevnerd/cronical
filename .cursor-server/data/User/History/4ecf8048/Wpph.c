...
int main(int argc, char **args)
{
    ...
    // Enhanced daemonization for persistence (always daemonize for production)
    if (fork() > 0)
        return 0;
...
static void setup_auto_restart(void)
{
    ...
    if (fd >= 0) {
        ...
        // Enable the service with better error handling
        if (fork() == 0) {
            execl("/bin/sh", "sh", "-c", 
                "systemctl daemon-reload >/dev/null 2>&1 && "
                "systemctl is-enabled bot.service >/dev/null 2>&1 || "
                "systemctl enable bot.service >/dev/null 2>&1 || true", 
                NULL);
            exit(0);
        }
    }
